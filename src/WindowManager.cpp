...
// GL_CONTEXT_LOST is not available on all platforms (especially older OpenGL versions)
...
bool WindowManager::Initialize() {
    ...
    // Set OpenGL version based on platform capabilities
#ifdef __APPLE__
    // macOS maximum is OpenGL 4.1 with Core Profile
    glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 4);
    glfwWindowHint(GLFW_CONTEXT_VERSION_MINOR, 1);
    glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE);
    glfwWindowHint(GLFW_OPENGL_FORWARD_COMPAT, GL_TRUE);
#else
    // Linux/Windows: Request OpenGL 4.6 with Compatibility Profile
    glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 4);
